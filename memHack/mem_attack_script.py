    ...
    def getPassword(self):
        ...
        while True:
            for char in string.printable:
                ...
                self.setProtectMem(len(self.password), len(self.password))
                result = self.checkPassword(''.join(self.password))
                if result == 'Wrong':
                    self.password.pop()
...
'''
To get the password we need to guess and check each character until we reach the end of the password
We simply need to block the next memory index after the character we are currently guessing
to determine whether we are at the end of the password or not. This blocked memory index acts as a
sliding window as we correctly guess the characters.

To guess the character, I looped through all printable ascii characters and checked it.
If the password checker returned `Wrong`, it just moves onto the next printable character and tries again.
Once it hits a `SEG ERROR`, we know the current character is correct because the password checker
is now checking the next character. The blocked memory then moves over an index and we begin looping
through all printable characters again, but for the next password index.
'''